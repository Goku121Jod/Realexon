...
async def handle_release_command(message):
    ...
    try:
        ...
        await message.channel.send("⚠️ No response received. Deal canceled.")
        del channel_data[ch_id]
...
async def update_expiry_notification(channel):
    start_time = channel_data[channel.id]["start_time"]
    time_left = 3600 - int((discord.utils.utcnow() - start_time).total_seconds())
    ...
    if time_left <= 0:
        ...
        embed = discord.Embed(title="⏳ Time Remaining", description=f"{minutes} minutes {seconds} seconds", color=0xff0000)
        await channel.send(embed=embed)
